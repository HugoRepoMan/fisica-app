window.LearningPathController = {
    ...
    async sincronizarDatosDesdeBD() {
        try {
            ...
            const backendIds = Array.isArray(userData.lecciones_completadas_ids) 
    ...
    aplicarLogicaDesbloqueo(modulos) {
        ...
        // Usar array de lecciones completadas si existe, si no usar el contador
        const leccionesCompletadasIds = Array.isArray(userData.lecciones_completadas_ids) 
            ? userData.lecciones_completadas_ids 
            : [];
        const leccionesCompletadas = parseInt(userData.lecciones_completadas || 0);
        const nivelUsuario = parseInt(userData.nivel || 1);

        console.log(`🔓 [LearningPath] Lecciones completadas (IDs): `, leccionesCompletadasIds);
        console.log(`🔓 [LearningPath] Contador: ${leccionesCompletadas}, Nivel: ${nivelUsuario}`);

        return modulos.map((modulo, index) => {
            let estado;
            const nivelRequerido = modulo.nivel_requerido || 1;
            
            // Determinar si esta lección ya fue completada
            const leccionCompletada = leccionesCompletadasIds.includes(modulo.id) || leccionesCompletadas > index;
            ...
            console.log(`  Módulo ${modulo.id} (${modulo.titulo}): ${estado} [Nivel req: ${nivelRequerido}]`);
